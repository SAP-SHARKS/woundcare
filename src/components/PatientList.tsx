...
export default function PatientList({ organizationId, onSelectPatient }: Props) {
  ...
  return <div className="wt-screen">
    ...
    <div className="wt-kpis">
      <button type="button" className="danger" onClick={() => setFilter('Urgent')} aria-label="Show urgent patients"><p>Urgent</p><strong>2</strong><span>oldest unacknowledged 41 min</span></button>
      <button type="button" className="warning" onClick={() => setFilter('Needs review')} aria-label="Show patients needing review"><p>Needs review</p><strong>5</strong><span>SLA 24 h · 1 due today</span></button>
      <button type="button" onClick={() => setFilter('All')} aria-label="Show patient roster for check-ins due"><p>Check-ins due</p><strong>7</strong><span>2 overdue &gt; 48 h</span></button>
      <button type="button" className="success" onClick={() => setFilter('Improving')} aria-label="Show improving patients"><p>Improving</p><strong>9</strong><span>median −18% area / 2 wks</span></button>
    </div>
